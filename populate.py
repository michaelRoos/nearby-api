...
list = [
	...
	['Concert', 'Street Performance', 'Alternative', 'Classic', 'Rock', 'Pop', 'Hip-Hop/Rap']
]
...
for i in range(500):
	...
	temp_event.save()
	temp_event.categories.add(categories.objects.all().filter(title__icontains='All')[0])
	temp_event.categories.add(categories.objects.all().filter(title__icontains='Professional')[0])
	temp_event.categories.add(categories.objects.all().filter(title__icontains='Resu')[0])
	temp_event.start_time = None
	temp_event.end_time = None
	temp_event.save()
